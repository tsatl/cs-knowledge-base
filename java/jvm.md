...
```text
唯一没有规定 OutOfMemoryError 的运行时数据区域
```
...
> **JVM 主要研究 Java 程序如何被加载、如何使用内存、如何执行字节码，以及垃圾如何被回收。**
